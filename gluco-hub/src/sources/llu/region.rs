...
/// LibreLink Up regional API endpoints. Hostnames lifted verbatim from the
/// reference TypeScript port (`nightscout-librelink-up`,
/// `src/constants/llu-api-endpoints.ts`). LibreView publishes new regions
...
impl Region {
    ...
    /// Mirrors the reference implementation's case-insensitive lookup so
    /// payloads like `"eu"` or `"EU"` both resolve.
