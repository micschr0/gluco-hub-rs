...
//!
//! Notable choices:
//! - We do NOT send a `trend` integer field. Nightscout dashboards key on
//!   `direction` (the textual form), so the integer would be redundant at
//!   best and wrong at worst across NS versions.
...
/// Both omit from the JSON when `None`.
