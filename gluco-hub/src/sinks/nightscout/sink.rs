...
//! types.
//!
//! Pre-upload dedup mirrors the reference port
//! (`timoschlueter/nightscout-librelink-up`): each push first reads
//! Nightscout's most recent entry timestamp via
//! `GET /api/v3/entries?count=1` and drops every reading whose
//! `timestamp` is at or before that high-water mark. NS still dedupes
