...
**Docker-based actions**: Jobs pulling Docker actions (zizmor, grype)
MAY use `egress-policy: audit`. The Docker daemon performs additional
DNS resolution during image pull that is impractical to predict.
Document the reason in a comment. Non-Docker jobs MUST use `block`.
