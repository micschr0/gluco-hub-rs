...
/// LibreLink Up app version sent in the `version` header. Mirrors the
/// reference port's `LINK_UP_VERSION` default; LibreView occasionally
/// rejects older values, so this is overridable at runtime.
pub const DEFAULT_LLU_VERSION: &str = "4.16.0";

/// LibreLink Up product identifier. The reference port uses `llu.ios`; the
/// Android value (`llu.android`) is also valid but the iOS string is the
/// most widely deployed.
pub const LLU_PRODUCT: &str = "llu.ios";

/// User-Agent matching the reference port. LibreView's WAF inspects this
/// and silently 4xxs unknown agents.
...
mod tests {
    ...
    fn base_headers_carry_required_fields() {
        let h = base_headers("4.16.0");
        assert_eq!(h.get("product").unwrap(), "llu.ios");
        assert_eq!(h.get("version").unwrap(), "4.16.0");
