#!/usr/bin/env bash
#
# Smoke-test the cgm-bridge HTTP surface end-to-end against the
...
PORT="${CGM_BRIDGE_SMOKE_PORT:-18080}"
BIND="127.0.0.1:${PORT}"
BASE="http://${BIND}"
...
unset CGM_BRIDGE__SOURCE__LLU__EMAIL || true
unset CGM_BRIDGE__SOURCE__LLU__PASSWORD_ENV || true
unset CGM_BRIDGE__SOURCE__LLU__REGION || true

export CGM_BRIDGE__HTTP__BIND="${BIND}"
...
echo "==> building cgm-bridge (default features include mock-source)…"
cargo build --quiet

BIN="${REPO_ROOT}/target/debug/cgm-bridge"
if [ ! -x "$BIN" ]; then
